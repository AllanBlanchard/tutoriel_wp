...
Axiom Q_empty :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:Numbers.BinNums.Z)
    (e:Numbers.BinNums.Z),
  (e <= b)%Z -> is_sint32_chunk Mint -> ((L_sum Mint a b e) = 0%Z).
...
Axiom Q_range :
  ...
  (b < e)%Z -> is_sint32_chunk Mint -> is_sint32 x1 ->
  (((L_sum Mint a b x) + x1)%Z = (L_sum Mint a b e)).

Axiom Q_sum_separable :
  forall (Mint:addr -> Numbers.BinNums.Z) (array:addr)
    (begin:Numbers.BinNums.Z) (split:Numbers.BinNums.Z)
    (end1:Numbers.BinNums.Z),
  (split <= end1)%Z -> (begin <= split)%Z -> is_sint32_chunk Mint ->
  (((L_sum Mint array begin split) + (L_sum Mint array split end1))%Z =
   (L_sum Mint array begin end1)).

(* Why3 assumption *)
Definition P_unchanged (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (array:addr) (begin:Numbers.BinNums.Z)
    (end1:Numbers.BinNums.Z) : Prop :=
  forall (i:Numbers.BinNums.Z),
  let a := shift array i in
  (begin <= i)%Z -> (i < end1)%Z -> ((Mint1 a) = (Mint a)).
...
Proof.
  ...
  | [ _ : _ |- context [ (- (1) + (?e + 1))%Z ] ] => replace (- (1) + (e + 1))%Z with e by lia
  end.

  intros Mi Mi' a b e TMi' TMi U.
  induction e using Z_induction with (m := b).
  + repeat(rewrite Q_empty) ; auto ; lia.
  + repeat(rewrite <- Q_range with (e := (e + 1)%Z) ; auto ; [ norm | lia ]).
    assert (Mi' (shift a e) = Mi (shift a e)) by (apply U ; lia).
    rewrite IHe ; [ lia | ].
    intros i ; intros ; apply U ; lia.
Qed.
