...
Theorem wp_goal :
  ...
  is_sint32_chunk t1 -> is_sint32_chunk t -> P_sorted t1 a 0%Z i ->
  P_shifted t t1 a 0%Z i i1 -> P_sorted t a i1 (i + i1)%Z.
Proof.
  Require Import Psatz.

  intros M2 M1 array len s tM1 tM2 Sorted Shifted.
  unfold P_shifted in Shifted.
  intros i j ; intros.
  assert(Hi: (exists pi, i = s + pi)%Z) by (exists (i - s)%Z ; lia).
  assert(Hj: (exists pj, j = s + pj)%Z) by (exists (j - s)%Z ; lia).
  ...
  simpl in * ; simpl.
  subst.
  replace (offset array + (s + pi))%Z with (offset array + pi + s)%Z by lia.
  replace (offset array + (s + pj))%Z with (offset array + pj + s)%Z by lia.
  repeat rewrite <- Shifted by lia.
  replace (offset array + pi + 0)%Z with (offset array + pi)%Z by lia.
  replace (offset array + pj + 0)%Z with (offset array + pj)%Z by lia.
  apply Sorted ; lia.
Qed.
