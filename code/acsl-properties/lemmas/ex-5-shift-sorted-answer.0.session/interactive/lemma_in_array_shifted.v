...
Definition P_sorted (Mint:addr -> Numbers.BinNums.Z) (arr:addr)
  ...
  forall (i:Numbers.BinNums.Z) (i1:Numbers.BinNums.Z), (begin <= i)%Z ->
  (i1 < end1)%Z -> (i <= i1)%Z ->
...
Axiom Q_shifted_still_sorted :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (array:addr) (len:Numbers.BinNums.Z) (shift1:Numbers.BinNums.Z),
  is_sint32_chunk Mint -> is_sint32_chunk Mint1 ->
  P_sorted Mint1 array 0%Z len ->
  P_shifted Mint Mint1 array 0%Z len shift1 ->
  P_sorted Mint array shift1 (len + shift1)%Z.

(* Why3 assumption *)
Definition P_in_array (Mint:addr -> Numbers.BinNums.Z)
    (value:Numbers.BinNums.Z) (arr:addr) (begin:Numbers.BinNums.Z)
    (end1:Numbers.BinNums.Z) : Prop :=
  exists i:Numbers.BinNums.Z,
  (((Mint (shift arr i)) = value) /\ (begin <= i)%Z) /\ (i < end1)%Z.
...
Theorem wp_goal :
  ...
  P_in_array t1 i2 a 0%Z i -> P_shifted t t1 a 0%Z i i1 ->
  P_in_array t i2 a i1 (i + i1)%Z.
Proof.
  Require Import Psatz.
  
  intros M1 M2 array len s value tM2 tM1 tvalue In Shifted.
  inversion_clear In as [ loc HIn ].
  inversion_clear HIn as [ Hcell Hrange ].
  exists (loc + s)%Z ; split ; [|lia].
  replace (shift array (loc + s)) with (shift (shift array loc) s).
  - rewrite <- Shifted ; try lia.
    replace (shift (shift array loc) 0)%Z with (shift array loc)%Z.
    + split ; lia.
    + unfold shift ; simpl.
      replace (offset array + loc + 0)%Z with (offset array + loc)%Z by lia.
      reflexivity.
  - unfold shift ; simpl.
    replace (offset array + loc + s)%Z
       with (offset array + (loc + s))%Z by lia.
    reflexivity.
Qed.
