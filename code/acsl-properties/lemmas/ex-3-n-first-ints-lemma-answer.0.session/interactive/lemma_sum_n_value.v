...
Axiom L_sum_n_def :
  ...
  ((n <= 0%Z)%Z -> ((L_sum_n n) = 0%Z)) /\
  (~ (n <= 0%Z)%Z -> ((L_sum_n n) = (n + (L_sum_n ((-1%Z)%Z + n)%Z))%Z)).
...
Proof.
  ...
  + assert (i = 0%Z) by lia ; subst.
    apply L_sum_n_def in H0.
    lia.
  + assert (X: ~ (i+1 <= 0)%Z) by lia.
    apply L_sum_n_def in X.
    replace (- (1) + (i + 1))%Z with i in X by lia.
    rewrite X.
    rewrite Z.mul_add_distr_l with (n := 2%Z).
    rewrite <- IHi ; try lia.
Qed.
