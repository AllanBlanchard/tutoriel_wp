...
/*@ requires x > INT_MIN ;
    terminates \true ; // proved because there is no calls, nor loops.
    assigns \nothing ;
...
/*@ requires INT_MIN < b - a <= INT_MAX ;
    terminates \true ; // proved because the only call in this function terminates
    assigns \nothing ;
...
void distances(int const* a, int const* b, int* result, size_t len){
  /*@ loop invariant 0 <= i <= len ;
      ...
      loop variant len - i ; // we could not prove termination without this variant
  */
...
/*@ requires \valid(c);
    terminates \false ; // we cannot prove this: connexion might not terminate
    assigns *c ;
    ensures \result \in { 0 , 1 } ;
*/
