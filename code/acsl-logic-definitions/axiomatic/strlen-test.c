...
/*@ logic integer strlen(char* s) = (*s != '\0') ? 1 + strlen(s+1) : 0 ; */
...
/*@
  lemma t1{L}:
    ...
    0 <= n && strlen(s) == n ==> s[n] == '\0' ;
*/
/*@
  lemma t2{L}:
    \forall char*s, integer n ;
    0 <= n && strlen(s) == n ==> (\forall integer i ; 0 <= i < n ==> s[i] != '\0') ;
*/
/*@
  lemma t3{L}:
  \forall char*s, integer n ;
  0 <= n && (\forall integer i ; 0 <= i < n ==> s[i] != '\0') && s[n] == '\0' ==>
  strlen(s) == n ;
*/
