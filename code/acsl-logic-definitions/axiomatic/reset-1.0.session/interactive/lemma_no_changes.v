...
Axiom Q_zeroed_empty :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:Numbers.BinNums.Z)
    (e:Numbers.BinNums.Z),
  (e <= b)%Z -> is_sint32_chunk Mint -> P_zeroed Mint a b e.

Axiom Q_zeroed_range :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:Numbers.BinNums.Z)
    (e:Numbers.BinNums.Z),
  let x := ((-1%Z)%Z + e)%Z in
  let x1 := Mint (shift a x) in
  is_sint32_chunk Mint -> is_sint32 x1 ->
  ((b < e)%Z -> (x1 = 0%Z) /\ P_zeroed Mint a b x) <-> P_zeroed Mint a b e.

(* Why3 assumption *)
Definition P_same_elems (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (a:addr) (b:Numbers.BinNums.Z)
...
Proof.
  ...
  induction e using Z_induction with (m := b) ; intros Same.
  + apply Q_zeroed_empty ; auto.
  + apply Q_zeroed_range ; auto.
    intros He ; apply Q_zeroed_range in H ; auto.
    replace (- (1) + (e + 1))%Z with e in * by lia.
    split.
    - replace (Mi' (shift arr e)) with (Mi (shift arr e)) by (apply Same ; lia).
      apply H.
    - apply IHe.
      * apply H.
      * intros i ; intros ; apply Same ; lia.
Qed.
