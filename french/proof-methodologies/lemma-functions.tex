...
est vrai. Maintenant, nous pouvons utiliser cette \textit{lemma function} en
...
à la précondition de la \textit{lemma function} (et qui est trivialement vraie,
...
de la \textit{lemma function} (et nous pouvons utiliser cette propriété comme
...
utilisons un appel à la précédente \textit{lemma function} pour rendre la preuve
...
Finalement, présentons un dernier usage des \textit{lemma functions}. Dans tous les 
...
L'utilisation des \textit{lemma functions} nous permet de raisonner par
...
Les \textit{lemma functions} ne nous fournissent pas de mécanisme équivalent
...
lemma en le remplaçant par une \textit{lemma macro}.
...
\textit{lemma functions} sont ensuite appelées dans d'autres fonctions pour déduire
...
La principale limitation des \textit{lemma functions} et \textit{lemma macros}
...
\textit{lemma function} correspondante, le type original de la valeur
\CodeInline{len} est un type entier mathématique, tandis que dans la
\textit{lemma function}, ce type est \CodeInline{size\_t}. Cela signifie que là
...
Nous ne pouvons pas écrire une \textit{lemma function} avec du code de preuve
...
L'autre limitation est liée aux \textit{lemma macros} et ce que nous avons déjà
...
\textit{lemma macros} peut générer beaucoup de code et d'annotations et amener
...
\begin{itemize}
    ...
    \item écrire (et utiliser) des \textit{lemma functions},
    \item écrire (et utiliser) des \textit{lemma macros}.
\end{itemize}
...
\textit{lemma functions} et \textit{macros} prouver que la plage complète est une
...
D'abord, définissons une \textit{lemma function} qui permet d'explicitement
...
En utilisant des \textit{lemma functions}, nous pouvons prouver que le lemme à
