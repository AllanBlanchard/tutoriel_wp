...
Nous obtenons la plus faible pré-condition: \CodeInline{1 + *q == 2}, et donc 
...
Maintenant, nous pouvons changer légèrement notre calcul de plus faible pré-condition
...
maintenant appliquer notre calcul de plus faible pré-condition à notre programme
précédent :
...
post-condition de la fonction \CodeInline{swap}, en forçant WP à ne pas simplifier la
propriété (\CodeInline{-wp-no-simpl}) :
...
\levelFourTitle{Branche ``then'' vide dans une conditionnelle}
