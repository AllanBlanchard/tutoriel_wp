...
\begin{CodeBlock}{c}
/*@
//pour tout "off" de type "integer", tel que SI "off" est compris entre 0 et "length"
//                                   ALORS la case "off" de "a" est différente de "element"
\forall integer off ; 0 <= off < length ==> a[off] != element;

//il existe "off" de type "integer", tel que "off" soit compris entre 0 et "length"
//                                   ET que la case "off" de "a" vaille "element"
\exists integer off ; 0 <= off < length && a[off] == element;
*/
\end{CodeBlock}
...
Une dernière information à propos des quantifications universelle et
existentielle : il est généralement préférable d'utiliser des types logiques, en
particulier pour les bornes d'entiers. Cela permet d'amener à des formules plus
simples (parce qu'il n'y a pas besoin d'encoder une contrainte de type sur les
variables ainsi quantifiées). La principale exception est quand la variable est
supposée représenter le contenu d'une cellule mémoire, et donc qu'il est
pertinent de la contraindre à ce qui peut effectivement exister dans la mémoire
en question.
...
\begin{CodeBlock}{c}
  //@ \forall integer off ; 0 <= off < length ==> array[off] != element;
\end{CodeBlock}
La variable qui atteint \CodeInline{length} à la fin de la boucle est
\CodeInline{i}, donc :
\begin{CodeBlock}{c}
  //@ loop invariant \forall integer off ; 0 <= off < i ==> array[off] != element;
\end{CodeBlock}
...
\begin{CodeBlock}{c}
//@ loop invariant \forall integer j; 0 <= j < 0 ==> array[j] != element;
\end{CodeBlock}
