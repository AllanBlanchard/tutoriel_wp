...
Les propriétés spécifiées dans les différents cas sont considérées comme étant
toujours vraies, donc si nous introduisons des propriétés permettant de prouver
\CodeInline{false}, nous sommes en mesure de prouver n'importe quoi. Nous
parlerons plus longuement de ce sujet dans la
Section~\ref{l2:acsl-logic-definitions-axiomatic} sur les définitions
axiomatiques. Les définitions inductives ont une contrainte syntaxique
qui a pour but d'éviter les problèmes d'inconsistance, mais nous verrons que
cela ne protège pas de tout.


La contrainte syntaxique sur les inductifs vise à assurer que les prédicats sont
bien fondés. Cela peut être fait en restreignant syntaxiquement ce que nous
acceptons dans une définition inductive en nous assurant que chaque cas est de
la forme :
\begin{CodeBlock}{c}
/*@
...
négation \CodeInline{!}) dans les différentes hypothèses \CodeInline{hx}.
Intuitivement, cela assure que nous ne pouvons pas construire des occurrences à
la fois positives et négatives de \CodeInline{P} pour un ensemble de paramètres
donnés (ce qui serait incohérent).
...
apparaît négativement à la ligne~8. Donc Frama-C lève une erreur :
...
Cependant, cela n'empêche pas d'écrire une propriété qui contient des termes
pouvant mener à une inconsistance. Par exemple, la définition suivante respecte
la contrainte syntaxique :
\CodeBlockInput[5][10]{c}{even-bad-2.c}
mais le prédicat \CodeInline{\textbackslash{}valid(p) \&\& p \textbackslash{}null}
est toujours faux, donc la propriété est équivalente à
\CodeInline{\textbackslash{}true} qui ne respecte pas la contrainte syntaxique.
Selon la manière dont WP simplifie la définition inductive, Why3 pourrait
détecter ces problèmes, puisque WP compile les définitions inductives ACSL en
définition inductives Why3, qui impose les mêmes contraintes syntaxiques. Par
exemple, la définition précédente est en effet simplifiée par WP, amenant à
l'erreur suivante :
\image{why3-failed-error}


Mais cela ne capture pas toutes les erreurs de ce type, la définition suivante
empêche WP ne produire la simplification, amenant donc à une définition
inconsistante permettant de prouver la clause
