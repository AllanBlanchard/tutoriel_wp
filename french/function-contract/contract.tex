...
\CodeInline{f} ce n'est pas le cas puisqu'elle pourrait être appelée dans un
autre fichier C où \CodeInline{h} aurait été modifiée par exemple.
...
considérer toutes les fonctions (incluant donc \CodeInline{main}) à être appelée
dans le programme. Donc, quand nous activons cette option sur la ligne de
commande de Frama-C, pour notre exemple, aucune condition de vérification n'est
générée pour les clauses \CodeInline{requires} de \CodeInline{main} puisqu'elle
n'est pas appelée et aucune des assertions n'est prouvée :
