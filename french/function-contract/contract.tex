...
typiquement les types mathématiques entiers et réels.
\end{itemize}
...
indiquant que la spécification est bien assurée par le programme.


\image{1-abs-3}


Il est possible de prouver les propriétés une à une en cliquant-droit sur
celles-ci et pas sur le nom de la fonction. Nous pouvons également voir que deux
...
Il y a plusieurs manières de demander à WP la génération de ces gardes. Tout
d'abord, il est possible de faire un clic-droit sur le nom de la fonction et
de sélectionner « Populate WP RTE guards ». Cela ajoutera les annotations liées
dans la fonction et l'on peut relancer WP sur celle-ci pour les vérifier.


\image{1-abs-generate-rte}


Il est aussi possible de les générer automatiquement en configurant ce paramètre
dans la barre latérale dédiée à WP. Ce composant permet aussi de personnaliser
les alarmes à générer, par exemple pour enlever certaines alarmes liées aux
comportements indéterminés ou pour ajouter de nouvelles alarmes pour des
comportements qui sont définis, mais qui peuvent causer des problèmes (par
exemple les débordements d'entiers non signés). Notons que certains paramètres
ne peuvent pas être changés ici, car ils ne sont actuellement pas supportés par
WP.


\image{1-abs-rte-sidebar}


Finalement, il est aussi possible de configurer l'auto-génération avec le
paramètre de ligne de commande \CodeInline{-wp-rte} qui activera la génération
dès le lancement de Frama-C. Cela peut également être fait après avoir lancé la
GUI de Frama-C, soit en cliquant sur l'icône d'engrenage en haut de l'interface,
soit en passant par le menu « Edit », « Frama-C Parameters », « WP »
\image{plugin-options}
où nous pouvons activer l'option \CodeInline{-wp-rte}. Notons que ce paramétrage
n'est pas sauvegardé. La manière généralement préférée est donc de fournir
l'option \CodeInline{-wp-rte} directement sur la ligne de commande.
...
pour les pastilles : vert + jaune et jaune.
...
le prouveur pour valider $G$ (et en fait, la majorité des prouveurs ne peuvent
même pas les lister). Donc si $S_3$ fait partie des suppositions, et si
...
La couleur jaune nous signale qu'aucun prouveur n'a pu déterminer si la
...
temps. Auquel cas, il rencontre un \textit{timeout} dont la durée est
configurable globalement (une nouvelle fois dans les paramètres de WP, section
« Prover Interface »).
\end{itemize}


Dans la vue WP, le panneau « \textit{WP Goals} » nous affiche la liste des
conditions de vérification et pour chaque prouveur indique un petit logo si la
preuve a été tentée et si elle a été réussie, échouée ou a rencontré un
\textit{timeout}. Notons qu'il est possible de filtrer cette liste (voir logo
en surbrillance dans la capture d'écran).
...
\CodeInline{postcondition 'positive\_value,function\_result'}, nous pouvons
d'ailleurs noter que lorsqu'une propriété est sélectionnée dans le tableau, elle
est également surlignée dans le code source. Les propriétés anonymes se voient
assignées comme nom le type de propriété voulu. La troisième colonne indique si
un script de preuve a été créé pour cette condition de vérification, pour
l'instant cette fonctionnalité est exclue de ce tutoriel.


Finalement, la dernière colonne fournit le résultat de la preuve : si la
propriété a été prouvée et les différents prouveurs qui ont été utilisés.

Certaines propriétés peuvent être prouvées parce que certaines sections de code
sont inatteignables. Dans ce cas, un prouveur particulier appelé CFG
(\textit{Control Flow Graph}, Graphe de Flot de Contrôle) est indiqué, c'est par
exemple le cas ici pour les clauses \CodeInline{exits} et
\CodeInline{terminates} : puisqu'aucune opération atteignable ne risque
d'appeler la fonction \CodeInline{exit} ou de créer une exécution infinie
(pas de boucle, ni d'appel de fonction), elles sont prouvées trivialement.

Pour les autres propriétés, WP doit générer des conditions de vérification. Pour
celles-ci, le premier prouveur est souvent Qed. Ce n'est pas réellement un
prouveur, mais un outil utilisé par WP pour simplifier les propriétés avant de
les envoyer réellement aux prouveurs externes, il se peut que cet outil soit en
mesure de résoudre le problème par lui-même, dans ce cas, aucun autre prouveur
n'est appelé. Sinon, les prouveurs utilisés sont listés. Par exemple ici,
Alt-Ergo qui est l'un des prouveurs que nous utilisons. Notons que pour la
propriété liée à RTE, un « timeout » est indiqué, ce qui signifie que le
prouveur s'est arrêté, car la preuve lui a pris trop de temps.
...
les options surlignées en vert sont désactivées) :
...
surligné en bleu dans la capture d'écran précédente), nous pouvons donc voir
...
La clause \CodeInline{requires} au point d'appel peut être cachée
individuellement (icône surlignée en bleu) ou globalement (icône surlignée en
vert). Cela peut être utile quand la quantité d'annotations devient importante
et que l'on doit se concentrer sur une partie d'entre elles.
...
\levelThreeTitle{ACSL : une logique totale}


Revenons à un détail à propos de la vérification d'absence d'erreurs à
l'exécution avec un autre exemple. La fonction suivante retourne la somme de
\CodeInline{a} et \CodeInline{b} :


\CodeBlockInput[5]{c}{addition-int.c}


À nouveau, si nous essayons de vérifier cet exemple avec l'absence d'erreurs à
l'exécution, nous devons prouver des propriétés avant l'instruction. Cette fois
les propriétés sont :
...
Et nous pouvons propager ces contraintes dans la clause \CodeInline{requires} :


\CodeBlockInput{c}{addition-int-fixed.c}
...
pour un calcul, auquel cas, c'est la valeur du champ qui se trouve promue). Si
le calcul comprend un \CodeInline{integer} et un \CodeInline{real}, la valeur
de type \CodeInline{integer} est promue en \CodeInline{real}.


Par conséquent, ici, même si \CodeInline{x+y} pourrait provoquer un dépassement,
nous pouvons comparer de manière sure la valeur obtenue à \CodeInline{INT\_MIN}
ou \CodeInline{INT\_MAX}. Notons que cela signifie aussi que pour les types C
dont le dépassement est défini par la norme, le comportement est celui d'ACSL.
Par exemple :


\CodeBlockInput{c}{addition-unsigned.c}


Ici, même si le type \CodeInline{unsigned} garantit que le dépassement entraîne
un « bouclage », ce n'est pas le cas en ACSL, et donc le contrat ne peut pas
être vérifié :


\image{addition-unsigned-failed}


Il est possible de forcer une conversion vers tout type C, qui permet pour les
entiers C, de forcer le bouclage :
...
rien ne nous permet de connaître les valeurs qui pourraient éventuellement être
calculées.


Cela fait d'ACSL une logique totale : toute opération est bien définie même si
le résultat peut ne pas être calculable. Cela inclut également les accès mémoire
qui peuvent être invalides en C, les accès à des éléments de listes logiques qui
n'existent pas, les calculs qui ne terminent pas ...
...
\levelFourTitle{Modulo}


Écrire la postcondition de la fonction de modulo suivante :


\CodeBlockInput[5]{c}{ex-1-modulo.c}
...
\end{CodeBlock}
qui devrait échouer. Adapter le contrat en ajoutant les bonnes préconditions.
Indice : il y a une condition à propos de \CodeInline{b} (seule) pour l'une des
alarmes, et pour la seconde alarme, une condition à propos de \CodeInline{a}
(seule) est suffisante pour lever l'alarme.
