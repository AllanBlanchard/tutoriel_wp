...
pas être satisfaites. Ils peuvent être activés depuis la ligne de commande avec
l'option \CodeInline{-wp-smoke-tests} ou via la barre latérale dédiée à WP (qui
peut également être utilisée pour personnaliser ces tests, mais nous n'entrerons
pas dans ces détails ici).


\image{2-sidebar-smoke}


Par exemple, ici, nous pouvons lancer cette ligne de commande :
...
Nous pouvons voir une pastille jaune et rouge à côté de la précondition de la
