...
the solvers that decide to stop or not after N unfold operations on the
predicates.
...
The properties specified in the different cases are always considered to be true,
so if we introduce a property that allows us to prove \CodeInline{false}, we are
able to prove everything. We give more details on this topic in
Section~\ref{l2:acsl-logic-definitions-axiomatic} about axiomatic definitions.
Inductive definitions have a syntactic constraint that is meant to avoid
consistency problems, even if we will see that it does not protect from
everything.


Basically, this constraint is meant to be sure that inductive predicates are
well-founded. This can be done by syntactically restricting what we allow in an
inductive definition, by making sure that each case has the form:
\begin{CodeBlock}{c}
/*@
...
is, \CodeInline{P} is not negated with \CodeInline{!}) in the
different hypotheses \CodeInline{hx}. Basically, this ensures that we cannot
build both positive and negative occurrences of \CodeInline{P} for some
parameters which would be incoherent.
...
appears negatively on line~8. Thus, Frama-C raises an error:


\begin{CodeBlock}{text}
[kernel:annot-error] code/acsl-logic-definitions/inductive/even-bad.c:8: Failure:
  ill-formed inductive definition: even_natural should not be used as
  negative occurrence. Ignoring global annotation
\end{CodeBlock}


However, it does not prevent from writing a property containing terms that can
lead to an inconsistent definition. For example, the following definition
respects the syntactic constraint:
\CodeBlockInput[5][10]{c}{even-bad-2.c}
but the predicate \CodeInline{\textbackslash{}valid(p) \&\& p \textbackslash{}null}
is always false, thus the property is equivalent to
\CodeInline{\textbackslash{}true} that does not respect the syntactic
constraint. Depending on how WP simplifies the inductive definition, Why3 might
detect these problems since WP compiles ACSL inductive definitions to Why3
inductive definitions, which have the same syntactic constraint. For example,
our previous definition is indeed simplified by WP, leading to the following
error:
\image{why3-failed-error}


But, it cannot capture everything, this similar definition prevent simplification,
leading to an inconsistent definition. Thus, we are able to prove the
\CodeInline{ensures \textbackslash{}false}:


\CodeBlockInput[5][22]{c}{even-bad-3.c}


\image{even-bad-proved}


Here, we could detect the problem, since \CodeInline{-wp-smoke-tests} can detect
