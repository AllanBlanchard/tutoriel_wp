...
When we start to create axiomatic definitions, it is worth adding assertions or
postconditions requiring a proof of false that we expect to fail to ensure that
the definition is not inconsistent, or to use smoke tests. However, this is
often not enough! If the subtlety that creates the inconsistency is hard enough
to find, provers could need a lot of information other than the axiomatic
definition itself to be able to find and use the inconsistency, so we need to
always be careful!


Note that unlike inductive definitions, where syntactic constraints can help
avoid creating only well-formed definitions, axioms do not have any constraints.
And it is the same for the Why3 backend, where such a definition is translated
into a list of axioms that are thus assumed.


\levelFourTitle{Effect of the \CodeInline{reads} clause}


Specifying the values read by a function or a predicate is important for the
consistency of an axiomatic definition. Indeed, as previously explained, if we
do not specify what is read when a pointer is received, an update of a value in
the array does not invalidate a property known about the content of the array.
In such a case, the proof is performed but since the axiom does not talk about
the content of the array, we do not prove anything.
...
\levelFourTitle{ACSL vs. C typing}


Another common mistake with axiomatic definition is to state that the result of
a logic function returning a C type corresponds to some calculation that is
performed on logic types and might be out of range. For example, if we take
our previous example on the factorial definition, and instead of an
\CodeInline{integer} type we use an \CodeInline{int} type, the definition
becomes inconsistent.

\CodeBlockInput[7][17]{c}{wrong-facto.c}


Indeed, since all integer calculation use mathematical integers, when
\CodeInline{i} is big enough, the term \CodeInline{i * factorial(i-1)} can
exceed \CodeInline{INT\_MAX}, while \CodeInline{factorial(i)} is always less or
equal to \CodeInline{INT\_MAX} (because of its type), which is inconsistent.
However, it is not that easy to make sure that SMT solvers detect the problem,
for example here, we add two additional lemmas to force solvers finding the
inconsistency:
...
Thus, one should avoid using basic C types for axiomatically defined values.
