...
\levelThreeTitle{Ghost code validity, what Frama-C verifies}
...
\CodeInline{break} is unreachable, an error will still be triggered:
...
Disallowing the ability to normal code to read ghost code has a very
...
Frama-C provides the \CodeInline{\textbackslash{}ghost} qualifier,
...
follows, while it is allowed for the \CodeInline{ghost} keyword:
...
locations when we initialize \CodeInline{p} to \CodeInline{\&a[0]}.
On the other hand, both the two following pointer initializations
are forbidden with the \CodeInline{\textbackslash{}ghost} qualifier:
...
\CodeInline{\&x}) to \CodeInline{p} and thus allow us, via the last
instruction to write \CodeInline{x} in ghost code. Consequently, such an
