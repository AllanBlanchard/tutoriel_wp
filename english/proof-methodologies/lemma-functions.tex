...
second invariant that shows that as we have not met any \CodeInline{v} since we
...
of our array, assuming that this second part is unchanged (when \CodeInline{i+1}
is less than \CodeInline{end} as else, we do not have anything to count):
...
a loop in order to prove this property. We advise to transform the invariant in
order to be sure that the property does not contain any division.
...
\CodeInline{fst} and \CodeInline{lst}. We provide the function and the contract.
Write the body of the function that allows to prove the postcondition. Note that
again, we advise to express the invariant without division.
...
occurrences of \CodeInline{v} in the array is comprised between 0 and its
\CodeInline{len}.
...
The function \CodeInline{occ\_monotonic} shoudl state that the number of
...
The function \CodeInline{occ\_pos\_find} should find an index of the value
\CodeInline{v} in the array provided that the number of occurrences of
\CodeInline{v} is positive. Note that you will probably need to use
\CodeInline{occ\_monotonic}.
...
For all these function, WP should be parameterized with the control of the
