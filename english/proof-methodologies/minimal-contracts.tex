...
programs correctly implement given specification. However, it is sometimes hard
...
breaches and thus, guaranteeing their absence already avoids a lot of attack
vector. The absence of runtime errors can be verified thanks to an approach
called minimal contracts.


\levelThreeTitle{Principle}


The minimal contracts approach is guided by the RTE plugins. Basically, the idea
is to generate the assertions related to absence of runtime errors for all the
function of a module or a project and to write the minimal set of (correct)
...
specify what is assigned by the function. Most of the time this annotation
cannot be avoided (which is expected in a language where everything is mutable
by default). Moreover, if one take the absolute value of an integer, it is
surely because they need a positive value. In reality, the minimal contract of
the absolute value function will be the following:
...
to express a precondition that the array is
\CodeInline{\textbackslash{}valid\_read} on the corresponding range of indices.
However, this is not enough since we have a loop in tis program, so we have to
provide a suitable invariant. We also probably want to prove that the loop
terminates.
...
to prove that the pointer is valid when it is not nul if the function does not
...
absence of runtime errors in all function of module or a program in (relative)
...
Prove the absence of runtime errors in the following programs using a minimal
contracts approach:
...
Do not forget to add the option \CodeInline{-warn-unsigned-overflow}.
