...
isolation from each other. We also see yellow bullets for \CodeInline{exits}
and \CodeInline{terminates} clauses. Since Frama-C generates these clauses
and does not have any way to verifying them, it warns the user that they should
check that the clauses are correct (or explicitly write them, so that the
Frama-C just assume that they are correct, just like the \CodeInline{ensures}
and \CodeInline{assigns} clauses).
