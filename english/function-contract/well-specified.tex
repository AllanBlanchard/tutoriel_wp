...
some preconditions are unsatisfiable. This can be enabled from the command line
with the option \CodeInline{-wp-smoke-tests} or via the WP sidebar (that can
also be used to customize the smoke tests but let us not detail this for now).


\image{2-sidebar-smoke}


For example, here, we can run this command:
...
We can see a red and yellow bullet on the precondition of the function,
...
presented up to now, and integrate the right \CodeInline{assigns} clause.
