...
\item
  ...
  than primitive C types, typically mathematical integers and reals.
\end{itemize}
...
specification is indeed ensured by the program.


\image{1-abs-3}


Furthermore, we can also prove properties one by one, by right-clicking on them
and not on the name of the function. We also see that now, two additional
annotations have been generated by Frama-C for our function:
\begin{itemize}
...
There are several ways to ask WP to generate these guards. First, one can
right-click on the function name and select ``Populate WP RTE guards''. This
will add the annotations in the function and again one can start WP on it to
verify them.
...
It is also possible to auto-generate them by setting up this parameter in the WP
sidebar. In this component, one can also customize the alarms to generate, for
example to remove some alarms related to undefined behaviors or to add new
alarms for behaviors that are defined, but that may cause problems (for example
unsigned overflows). Note that some parameters cannot be tuned since they are
currently not supported in WP.


\image{1-abs-rte-sidebar}


Finally, one can also configure the automatic generation using the command line
parameter \CodeInline{-wp-rte} that will enable it directly when starting Frama-C.
This can also be done after starting Frama-C GUI, either by clicking on the gear
button on top of the GUI or by selecting it in the `'`Edit'' menu:
``Frama-C Parameters'', ``WP''
\image{plugin-options}
where we can enable the \CodeInline{-wp-rte} option. Note that it is not saved.
Thus, providing the option \CodeInline{-wp-rte} on the command line is generally
...
validate $G$ (and in fact, most provers cannot even list them). So, if
$A_3$ is an assumption, and if WP did not succeed in getting a proof of $A_3$,
it indicates that $G$ is true, but only assuming that $A_3$ is true, for which
no proof has been established so far.



The yellow color indicates that no prover could determine if the
...
\item the prover did not have enough time to compute the proof and met a timeout
      which can be configured globally in the WP sidebar (Prover configuration,
      General configuration, Timeout) or in the parameters, (section
      ``Prover Interface'', \CodeInline{-wp-timeout}).
\end{itemize}


In the WP view, the ``WP Goals'' panel shows the list of verification
conditions, and for each prover the result is symbolized by a logo that
indicates if the proof has been tried and if it succeeded, failed or met a
timeout. Note that it is possible to filter goals (see highlighted icon in the
screenshot).
...
kind of wanted property. The third column indicates whether a proof script has
been created for the verification condition, for now this feature is excluded
from the tutorial.


Finally, the last column provides the proof results: whether the property has
been proved and the different provers that have been use.

Some properties might be proved because code sections are unreachable. In this
case, a particular prover named CFG (Control Flow Graph) is indicated, this is
for example the case here for the \CodeInline{exits} and \CodeInline{terminates}
clauses: since no reachable operation might trigger the \CodeInline{exit}
function nor create infinite execution (no loop, nor function call), they are
trivially proved.

For other properties, WP needs to generate an actual verification condition. For
these, the first ``prover'' is often Qed. This is not really a prover. It is a
tool used by WP to simplify properties before sending them to external provers,
it happens that sometimes it can solve goals by itself, in this case, no other
prover is called. Else, used provers are listed. For example here, Alt-Ergo
which is one of the solvers we use. Notice that for the RTE property, a timeout
is indicated which means that the  solver was stopped because it took too much
time.
...
verification condition (if this is not the case, make sure that the options
highlighted in green are turned off):
...
button highlighted in blue in the previous screenshot),
...
The inline \CodeInline{requires} clause can be collapsed either individually
(icons highlighted in blue) or globally (icon highlighted in green). It can be
useful when the quantity of annotations grows, and we want to focus on a few
of them.
...
\levelThreeTitle{ACSL: a total logic}


Let us come back on a detail about the verification of absence of runtime
errors with another example. The following function returns the sum of
\CodeInline{a} and \CodeInline{b}:


\CodeBlockInput[5]{c}{addition-int.c}


Again, if we try to verify this example with absence of runtime errors, we need
to prove properties before the instruction. This time, the properties are:
...
And we can for example propagate this constraint to the \CodeInline{requires}:


\CodeBlockInput{c}{addition-int-fixed.c}


Thus, the very same expression that may produce an undefined behavior in C can
be safely used for verification in ACSL. The reason for this is that C values
are automatically promoted to a logic type before computation. For C integer
...
Here, even if the \CodeInline{unsigned} type guarantees that in case of overflow
the computation wraps around, it is not the case in ACSL, thus the contract
cannot be verified:
...
One can force a cast to any C type, which will, for C integers, force the
wrapping behavior:
...
This makes of ACSL a total logic: any operation is well-defined even if the
result may not be computable. This also includes memory accesses that might
be invalid in C, access to logic lists elements that do not exist, computations
...
Write the postcondition of the following modulo function:
...
It should fail, adapt the contract by adding the right preconditions. Hint:
there is one condition about \CodeInline{b} alone for one of the alarms, and
one about \CodeInline{a} alone is enough to avoid the second one.
