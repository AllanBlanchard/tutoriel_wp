...
succeeded, failed or met a timeout. In the current example, we have just added
a try using Z3 to illustrate how a timeout is represented in the GUI (it is not
necessary to try this yourself on this example). Note that it
...
It is a tool by WP to simplify properties before sending them to externel
provers. Then we find Script, this is way to finish some proofs by hand when
