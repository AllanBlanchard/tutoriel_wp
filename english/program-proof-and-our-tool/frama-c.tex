...
Note that it is also possible to follow the installation steps provided for
Linux in Windows Subsystem for Linux.
...
\begin{Information}
  The author does not know if it works on Windows and Cygwin. With WSL, it
  should work.
\end{Information}
...
\begin{Warning}
  ...
  at all) to install Why3 on Windows. The author cannot be charged for
  injuries that could result of such an operation.
\end{Warning}


Why3 is a deductive proof platform developed by the LRI in Orsay. It
...
interactive provers. This point is the one that interests us here. WP can
translate the verification conditions to the Why3 language and then use
Why3 to interact with solvers.
