...
\item the program is not correct, or does not have a defined behavior,
      ...
      accesses out of the bounds of an array when searching the minimal
      value of the array),
\item the program is not correct according to the specification that we
      ...
      find the minimum value of an array, but in fact, due to an error, it
      does not check the last value of the array),
\item the specification does not exactly reflect what we had in mind for
      ...
      minimal value of the array, but we did not specified that if there
      are multiple ones, we need the position of the first one, because it
      seemed too straightforward, but consequently, this is not what the
      program does.
\end{itemize}
