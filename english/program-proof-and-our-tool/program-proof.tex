...
First, there is a difference between safety and security. Loosely speaking,
...
is a first before we can get security\footnote{Depending on the field you are
  ...
  if security is not guaranteed before.}. Second, and more important, we do not
really define what we mean by ``bug-free''. Creating software always relies at
...
specification'', since for example it is generally not the fault of the
specification if we try to access a nul pointer. Moreover, there also
exist approaches that allow to
