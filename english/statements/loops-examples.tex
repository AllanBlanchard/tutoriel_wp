...
\begin{CodeBlock}{c}
/*@
// for all "off" of type "integer", IF "off" is comprised between 0 and "length"
//                                  THEN the cell "off" in "a" is different from "element"
\forall integer off ; 0 <= off < length ==> a[off] != element;

// there exists "off" of type "integer", such that "off" is comprised between 0 and "length"
...
range of values, a property is true, either about all of them
...
One final note about universal and existential quantification: prefer using logic
types, in particular to talk about integers bounds. It generally leads to simpler
formulas (because there is no need to encode a type constraint for the quantified
variables). The main exception is when the variable is meant to represent the
content of something in memory, and thus that it is meaningful to constrain it
to what can really exist in memory.
