...
Before anything else, we write the \CodeInline{loop assigns} clause. It is
...
efficiently something like \CodeInline{array[x .. y]} when \CodeInline{x} or
...
will reach at the end of the loop. For example, if in the loop some \CodeInline{i}
...
an element such that it satisfies the property \CodeInline{p} at the same
time it also satisfies \CodeInline{q}. If we use an implication, as we do
