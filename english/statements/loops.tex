...
But if we run with the option \CodeInline{-wp-no-subst}, we can see
these proof details:
...
(we have used the option \CodeInline{-wp-no-qed} to avoid immediate proof by
Qed):
...
use \CodeInline{-wp-no-qed}, and the verification conditions have been slightly
cleaned up):
