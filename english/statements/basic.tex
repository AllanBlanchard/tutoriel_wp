...
meaning that two pointers can point on the same memory location. Our weakest
precondition calculus should consider this cases. For example, let us consider
...
does not allow us to conclude that the program is correct, since the formula
\CodeInline{p == q ==> *q == 1} cannot be verified. In fact, here, we would like
to be able to compute a property like;
...
Of course, at the beginning of a function, the memory context can be populated
with the memory locations for which a value is defined.
...
variable $M$ that models the memory.
...
\item we have to apply the rule of assignment for pointers, but for this we need
  to introduce $M$,
\item we replace pointer accesses in the formula by a call to select on $M$,
...
\item we perform some simplification to the goal ...
\item ... and finally conclude that either $M[q] = 1$ or $p = q$.
\end{enumerate}


Then in our program, since we now that $p = q$ we can conclude that the program
is correct.
...
fact WP creates multiple variables for memory. However, let us have a look to
the goal generated for the post-condition of the swap function, by preveting
WP to simplify the goal using the option \CodeInline{-wp-no-simpl}:


\image{memory-model}


And we can see, in the beginning of the proof obligation, that a variable
