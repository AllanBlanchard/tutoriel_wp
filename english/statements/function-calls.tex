...
Note that \CodeInline{n - 10 < n} is formulated \CodeInline{n_1 (= n) <= 9 + n}.
...
Note that the \CodeInline{decreases} expression is considered at
\CodeInline{Pre} state: the value of the expression when a recursive call is
done must be smaller that the value of the expression at \CodeInline{Pre} state.
...
\CodeBlockInput[5][8]{c}{terminates.c}
...
\CodeBlockInput[10][14]{c}{terminates.c}
...
\CodeBlockInput[16][22]{c}{terminates.c}
...
\CodeBlockInput[24][33]{c}{terminates.c}
...
\CodeBlockInput[35][43]{c}{terminates.c}
