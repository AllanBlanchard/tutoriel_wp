...
this postcondition is ignored at its call site. Finally, while \CodeInline{*z} is
...
its call site, yet we do not assume it when verifying \CodeInline{f}. Finally, when
...
at its call site. In the following example:

\CodeBlockInput[5]{c}{check-admit-requires.c}

\image{requires-CA}

We cannot prove that \CodeInline{R1} is true at its call site, yet we
assume it is in
\CodeInline{callee}, and can thus prove \CodeInline{A1}. And, while
\CodeInline{R2} is verified at its call site, we cannot prove \CodeInline{A2} in
\CodeInline{callee} since we ignore \CodeInline{R2} when verifying \CodeInline{f}.
Finally, we do not try to verify \CodeInline{R3} at its call site (and WP does not
...
Below is a general overview of the meaning of the keywords used
...
It is thus recommended indicating the precise behaviors that can lead
...
and at each call site, we use the \CodeInline{exit} clause as a postcondition, then
...
disabling goal simplifications (option \CodeInline{-wp-no-qed}, we removed
...
statement is \CodeInline{Here}. The value of the expression at the call site
must be less than the value of the expression in the \CodeInline{Pre} state.
...
\begin{Information}
  ...
  not cover function pointers in this tutorial, but one can refer to the clause
  \CodeInline{calls} in the Frama-C user manual.
\end{Information}
...
We get the following verification conditions (by using \CodeInline{-wp-no-qed}
