...
\CodeInline{ensures} clauses can be only checked or admitted.
...
when verifying \CodeInline{f}. On the opposite, when we call \CodeInline{f}, we
admit that it is true. Now, if \CodeInline{f} has a \CodeInline{check ensures}
clause, we have to verify it when verifying \CodeInline{f}, but we
\textit{ignore} it when \CodeInline{f} is called. On the opposite, if
\CodeInline{f} has an \CodeInline{admit ensures} clause, it is not verified when
verifying the \CodeInline{f}, still we admit it when it is called. In the
following example:
...
called, we admit that \CodeInline{E1} is true. We can prove the clause
\CodeInline{E2} because \CodeInline{*y} is assigned to 10 that is indeed greater
than or equals to 10. However, we cannot prove the check \CodeInline{A2} because
this postcondition is ignored at call point. Finally, while \CodeInline{*z} is
assigned to 20, we do not try to verify that \CodeInline{E3} is true (and it is
not), yet WP does not warn about it: it is admitted, thus at call point, we can
prove that \CodeInline{z} is greater than 30 even if it is not.

Now, we can present the behavior for the \CodeInline{requires} clause. When a
...
call point, yet we do not assume it when verifying \CodeInline{f}. Finally, when
a function \CodeInline{f} has an \CodeInline{admit requires} clause, it is
assumed to be true when verifying \CodeInline{f} without any verification of it
...
We cannot prove that \CodeInline{R1} is true at call point, yet we admit it in
\CodeInline{callee} thus can prove \CodeInline{A1}. On the opposite, while
\CodeInline{R2} is verified at call point, we cannot prove \CodeInline{A2} in
\CodeInline{callee} since we ignore \CodeInline{R2} when verifying \CodeInline{f}.
Finally, we do not try to verify \CodeInline{R3} at call point (and WP does not
warn about it even if it is false), but we can prove that \CodeInline{A3} is
...
the \CodeInline{exits} clause might be anything):
...
to a call to \CodeInline{exit} and to forbid call \CodeInline{exit} in the other
cases:
...
\CodeInline{return} statement created by Frama-C (although it is exactly like
we would start from all the different \CodeInline{return} statements in the
...
Instead of starting the calculus from \CodeInline{return} instructions, we start
a new calculus from each function call (as it might be one of the exit points)
and at this call, we use the \CodeInline{exit} clause as a postcondition, then
...
iteration, \CodeInline{decreases} represents an upper bound on the depth of the
...
instruction is \CodeInline{Here}. The value of the expression at call point must
be less than the value of the expression in \CodeInline{Pre} state.
